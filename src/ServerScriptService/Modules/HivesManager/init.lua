...
local RColors = {
	...
	["Rare"] = Color3.fromRGB(176, 193, 245),
...
function module.PopUp2(BeeModule, pui)
	...
	infos.Desc.Text = BeeModule.Description
...
function module.PopUp(Player, Bee, Item, Gifted, Slot, New, PData)
	...
	if PlayerGui:FindFirstChild("Screen"):FindFirstChild("BeePopUp") then
		PlayerGui:FindFirstChild("Screen"):FindFirstChild("BeePopUp"):Destroy()
	end
	local PopUpUI =  PopUp:Clone()
	local BeeModule = require(game.ReplicatedStorage.Bees[Bee])
	PopUpUI.Discovered.Visible = false
	PopUpUI.Parent = PlayerGui:FindFirstChild("Screen")
	...
	local BeeName = Bee
	if Gifted then -- ОДАРЁНКА
		print(PData.DiscoveredBees[BeeName].Gifted)
		if(not PData.DiscoveredBees[BeeName].Gifted) then
			print("Новый вид пчелы: "..BeeName.." | Одарённая")
...
local function ManageSlot(Player, Slot, Gifted, Bee)
	local PData = Data:Get(Player)
	local BeeData = PData.Bees[tonumber(Slot.Name)]
	local BeeModule = require(game.ReplicatedStorage.Bees[Bee])
	local DC = Slot.Dc
	Slot.Outline.Gui.Gifted.Visible = Gifted
	Slot.Transparency = 0
	
	DC.UI.Decal.Image = BeeModule.Icon
	Slot.Color = RColors[BeeModule.Rarity]
	Slot.BeeN.Value = Bee
...
function module.CreateSlot(Slot, Bee, Player, Gifted, BeeData, PData)
	local BeeModule = require(game.ReplicatedStorage.Bees[Bee])
	if Slot:FindFirstChild("Outline") then
		Slot:FindFirstChild("Outline"):Destroy()
	end
	if Slot:FindFirstChild("Dc") then
		Slot:FindFirstChild("Dc"):Destroy()
	end
	...
	ManageSlot(Player, Slot, Gifted, Bee)
end

--function module.ChangeBee(Slot, Bee, Player, Gifted, PData, OldBee)
--	workspace.PlayersE[Player.Name].Bees[Player.Name][Slot.Name]:Destroy()
--	ManageSlot(Player, false, DC, Slot, BeeModule, Gifted, Bee, false)
--end

function module.GetSlot(Player, Slot, Bee, Gifted)
	local OL = game.ServerStorage.Assets.Outline:Clone()
	local DC = game.ServerStorage.Assets.Dc:Clone()
...
function module.GetSlots(Player)
	local PData = Data:Get(Player)
	...
	for i, Slot in pairs(PData.Bees) do
		if Slot.BeeName ~= "" then
			module.GetSlot(Player, workspace.Hives[PData.Vars.Hive].Slots[i], Slot.BeeName, Slot.Gifted)
		end
	end
end

function module.NewSlot(PData, nowslot)
	...
	Template.Position = AllSlots[nowslot-4].Position + Vector3.new(0,3.07,0)
	Template.Parent = AllSlots
	--if (nowslot - 4) == 21 or (nowslot - 4) == 26 or (nowslot - 4) == 31 or (nowslot - 4) == 36 or (nowslot - 4) == 41 or (nowslot - 4) == 46 then
	--	AllSlots.Parent.Pad.Circle.Display.Position += Vector3.new(0,3,0)
	--end
end
